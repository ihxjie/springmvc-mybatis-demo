...
<head>
    ...
    <script type="text/javascript" src="${pageContext.request.contextPath }/assets/js/applyList.js"></script>
    <script type="text/javascript" src="${pageContext.request.contextPath }/assets/js/admin.js"></script>
    <script type="text/javascript" src="${pageContext.request.contextPath }/assets/js/deleteApplys.js"></script>
    <script type="text/javascript">
        function d(){
            window.location.href="${pageContext.request.contextPath}/sys/applyController/excel";
        }
    </script>
</head>
<body id="page-top">
            ...
            <nav class="navbar navbar-light navbar-expand bg-white shadow mb-4 topbar static-top">
                <div class="container-fluid"><button class="btn btn-link d-md-none rounded-circle mr-3" id="sidebarToggleTop" type="button"><i class="fas fa-bars"></i></button>
                    <div class="input-group">
                        <form method="post" action="${pageContext.request.contextPath }/sys/applyList/looklist.html">
                            <span>报名id：</span>
                            <input name="applyId" id="applyId" class="input-text"	type="text" value="${applyId}">
                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-6 text-nowrap">
                                <button><a name="delAllApply" id = "delAllApply" href="#" >批量删除</a></button>
                            </div>
                            <div class="col-md-6">
                                ...
                                <c:forEach var="tests" items="${pageInfo.list}" >
                                    <tr>
                                        <td>
                                            ...
                                            <span class = "id">${tests.applyId}</span>
                                        </td>
                                        <td>
                                            <span>${tests.projectId}</span>
                                        </td>
                                        <td>
                                            <span>${tests.personId}</span>
                                        </td>
                                        <td>
                                            <span>${tests.personName}</span>
                                        </td>
                                        <td>
                                            <c:if test="${tests.status==0}">否</c:if>
                                            <c:if test="${tests.status==1}">是</c:if>
                                        </td>
                                        <td>
                                        <c:if test="${tests.isStudent==0}">教师</c:if>
                                        <c:if test="${tests.isStudent==1}">学生</c:if>
                                        </td>
                                        <td>
                                            <span><a class="viewApply" href="javascript:;" applyId=${tests.applyId} ><img src="${pageContext.request.contextPath }/assets/img/read.png" alt="查看" title="查看"/></a></span>

                                            <span><a class="modifyApply"  href="javascript:;" applyId=${tests.applyId} ><img src="${pageContext.request.contextPath }/assets/img/xiugai.png" alt="修改" title="修改"/></a></span>
                                        </td>
                                    </tr>
                                </c:forEach>
                            </table>
                            ...
                            <div class="col-md-6 offset-md-4">
                                <nav aria-label="Page navigation example">
                                    <ul class="pagination pagination-sm">
                                        <li class="page-item"><a class="page-link" href="${BASE_PATH}/springmvc_mybatis_demo_war/applyList?page=1">首页</a></li>
                                        <c:if test="${pageInfo.hasPreviousPage}">
                                            <li class="page-item"><a class="page-link"
                                                                     href="${BASE_PATH}/springmvc_mybatis_demo_war/applyList?page=${pageInfo.pageNum-1}">上一页</a></li>
                                        </c:if>
                                        <c:forEach items="${pageInfo.navigatepageNums}" var="page">
                                            ...
                                            <c:if test="${page!=pageInfo.pageNum}">
                                                <li class="page-item"><a class="page-link"
                                                                         href="${BASE_PATH}/springmvc_mybatis_demo_war/applyList?page=${page}">${page}</a></li>
                                            </c:if>
                                        </c:forEach>
                                        <c:if test="${pageInfo.hasNextPage}">
                                            <li class="page-item"><a class="page-link"
                                                                     href="${BASE_PATH}/springmvc_mybatis_demo_war/applyList?page=${pageInfo.pageNum+1}">下一页</a></li>
                                        </c:if>
                                        <li class="page-item"><a class="page-link" href="${BASE_PATH}/springmvc_mybatis_demo_war/applyList?page=${pageInfo.pages}">末页</a>
                                        </li>
                                    </ul>
                                </nav>
                            </div>
